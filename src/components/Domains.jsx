...
import { AddDomainModal, EditDomainModal } from './Modal'

export default function Domains({ poles, domains, projects, settings, updateDomain, addDomain, removeDomain, collapsed, toggleCollapsed }) {
  ...
  return (
    <div className="container">
      ...
      {poles.map(pole => {
        ...
        const rc = { G:0,O:0,R:0 }; pDomains.forEach(d=>rc[d.rag]++)

        return (
          <div key={pole.id}>
            <div className="pole-header" style={{borderLeftColor:pole.color,cursor:'pointer',userSelect:'none'}} onClick={()=>toggleCollapsed(pole.id)}>
              <span style={{fontSize:18}}>{pole.icon}</span>
              <div style={{flex:1}}>
                <div className="pole-name">{pole.name}</div>
                <div className="pole-sub">
                  {pDomains.length} domaines
                  {isCollapsed && <span style={{marginLeft:8}}>
                    {rc.G>0&&<span style={{color:'#22c55e',marginRight:4,fontSize:10}}>●{rc.G}</span>}
                    {rc.O>0&&<span style={{color:'#f59e0b',marginRight:4,fontSize:10}}>●{rc.O}</span>}
                    {rc.R>0&&<span style={{color:'#ef4444',fontSize:10}}>●{rc.R}</span>}
                  </span>}
                </div>
              </div>
              <div style={{textAlign:'right',minWidth:100}}>
                <div style={{fontFamily:'DM Mono,monospace',fontSize:20,fontWeight:700,color:pole.color}}>{poleDays}<span style={{fontSize:11,color:'#6b7a99'}}>j</span></div>
                <div className="progress-bar" style={{width:90,marginLeft:'auto',marginTop:3}}><div className="progress-fill" style={{width:`${polePct}%`,background:pole.color}}/></div>
                <div style={{fontSize:10,color:'#6b7a99',marginTop:2}}>{polePct}%</div>
              </div>
              <div style={{marginLeft:8,fontSize:14,color:'#6b7a99',transition:'transform .2s',transform:isCollapsed?'rotate(-90deg)':'rotate(0deg)'}}>▾</div>
            </div>
                ...
                {pDomains.map(d => {
                  ...
                  return (
                    <div key={d.id} className="domain-card" style={{borderLeftColor:rag.dot}}>
                      <div className="domain-header">
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:'flex',alignItems:'center',gap:6}}>
                            ...
                            <button className="edit-btn" title="Modifier" onClick={e=>{e.stopPropagation();setEditDomain(d)}}>✏</button>
                          </div>
                          <div className="domain-sub">{d.sub}</div>
                        </div>
                        <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',gap:4}}>
                          <button className="rag-btn" style={{background:rag.bg,color:rag.color}} onClick={()=>updateDomain(d.id,{rag:cycleRag(d.rag)})}>
                      ...
                      <div className="domain-comment" contentEditable suppressContentEditableWarning onBlur={e=>updateDomain(d.id,{comment:e.target.innerText.trim()})}>{d.comment||''}</div>
                      <div className="domain-days-row">
                        <span className="domain-days-lbl">Jours</span>
                        <input type="range" min="0" max="12" step="0.5" value={d.days} onChange={e=>updateDomain(d.id,{days:parseFloat(e.target.value)})} style={{flex:1,accentColor:'#d4a843'}}/>
                        <span className="domain-days-val">{d.days}j</span>
                      </div>
                      {projs.length>0&&(
                        <div className="proj-chips">
                          {projs.map(p=>{
                            const ph=getPhase(p.pct); const pr=RAG[p.rag]||RAG.G
                            return (
      ...
      {editDomain&&<EditDomainModal domain={editDomain} poles={poles} onSave={(id,patch)=>updateDomain(id,patch)} onClose={()=>setEditDomain(null)}/>}
    </div>
  )
}
